...
class AuthController{
  ...
  static function viewlogin(){
    loadView('Auth/LoginPage');
  }

  static function viewregister(){
      loadView('Auth/RegisterPage');
  }

  static function login(){
    global $url;
    if (empty($_POST["email"])) {
      echo "<script>alert('email tidak boleh kosong');window.location.href = '".$url."/login'</script>";
      exit(); 
    } 
    ...
    if (strlen($_POST["password"]) < 8) {
      echo("<script>alert('Password minimal input 8 karakter');window.location.href = '".$url."/login'</script>");
      exit();
    }
    $data = AuthModel::getdata($_POST["email"]);
    if ($_POST["email"] != $data[0]['email']) {
      echo("<script>alert('Email anda salah');window.location.href = '".$url."/login'</script>");
      exit();
    }
    if ($_POST["password"] != $data[0]['password']) {
      echo("<script>alert('Password anda salah');window.location.href = '".$url."/login'</script>");
      exit();
    }
    session_start();
